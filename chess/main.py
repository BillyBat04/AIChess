...
DIMENTION = 8
BAR_HEIGHT = 50
BOARD_SIZE = HEIGHT - BAR_HEIGHT
SQ_SIZE = BOARD_SIZE // DIMENTION
MAX_FPS = 15
IMAGES = {}
...
def main():
    ...
    while running:
        if showMenu:
            ...
            for e in p.event.get():
                if e.type == p.QUIT:
                    ...
                    if resetButtonRect.collidepoint(location):
                        ...
                        row = (location[1] - BAR_HEIGHT) // SQ_SIZE
                        ...
                        if len(playerClicks) == 2:
                            ...
                            for i in range(len(validMoves)):
                                if move == validMoves[i]:
                                    gs.makeMove(validMoves[i], False)
                                    moveMade = True
                                    animate = True
                                    sqSelected = ()
                                    playerClicks = []
            ...
            drawButton(controlBar, 'Reset', resetButtonRect)
            drawButton(controlBar, 'Undo', undoButtonRect)
...
def drawBoard(screen):
    ...
    for r in range(DIMENTION):
        for c in range(DIMENTION):
            color = colors[(r + c) % 2]
            p.draw.rect(screen, color, p.Rect(c * SQ_SIZE, r * SQ_SIZE + BAR_HEIGHT, SQ_SIZE, SQ_SIZE))
...
def drawPieces(screen, board):
    for r in range(DIMENTION):
        for c in range(DIMENTION):
            piece = board[r][c]
            if piece != "--":
                screen.blit(IMAGES[piece], p.Rect(c * SQ_SIZE, r * SQ_SIZE + BAR_HEIGHT, SQ_SIZE, SQ_SIZE))
